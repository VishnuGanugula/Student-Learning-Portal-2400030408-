...
export default function App() {
  ...
  const renderCurrentView = () => {
    switch (currentView) {
      ...
      case "profile":
        // render specialized student profile component for students
        if (userInfo?.role === "student") {
          return <StudentProfile userId={userInfo?.id} userName={userInfo?.name} />;
        }
        return (
          <div className="p-6 space-y-6">
            ...
            <div className="bg-card p-6 rounded-lg border">
              ...
              <div className="space-y-2">
                <div>
                  <strong>{userInfo?.role === 'faculty' ? 'Faculty ID' : 'Admin ID'}:</strong> {userInfo?.id}
                </div>
                <div>
